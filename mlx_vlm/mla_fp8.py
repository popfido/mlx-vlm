...
def mla_fp8_enabled() -> bool:
    ...
    return os.environ.get(MLA_FP8_ENV, "0").strip().lower() in ("1", "true", "yes", "on")
...
class Fp8MLAKVCache:
    ...
    def _grow(self, B: int, D: int, ng: int, R: int, L: int, dtype: mx.Dtype):
        ...
        if self.latent_codes is not None:
            ...
            self.latent_scales = mx.concatenate([self.latent_scales, new_scales], axis=1)
...
def _partial_kernel():
    ...
    _PARTIAL_KERNEL = mx.fast.metal_kernel(
        ...
        input_names=["q", "pe", "latent_codes", "latent_scales", "lut", "meta", "scale"],
...
def dequantize_latent_fp8_metal(
    ...
    out = _dequant_kernel()(
        inputs=[codes, scales, fp8_lut(), mx.array([total, D, ng, group_size], mx.int32)],
...
def _mla_fp8_decode_sdpa(
    ...
    latent = dequantize_latent_fp8_metal(latent_codes, latent_scales, group_size, q.dtype)
...
def mla_fp8_decode(
    ...
    if method in ("kernel", "sdpa") and not _metal_available():
        method = "mlx"
    if method == "kernel":
        return _mla_fp8_decode_metal(q, pe, latent_codes, latent_scales, scale, group_size)
    if method == "sdpa":
        return _mla_fp8_decode_sdpa(q, pe, latent_codes, latent_scales, scale, group_size)
