...
def memory_table(group=DEFAULT_GROUP_SIZE):
    ...
    print("| model | expanded bf16 (current mlx-vlm) | absorbed bf16 latent | "
    ...
    for name, layers, H, lora, nope, rope, v in CONFIGS:
        ...
        gb = lambda b: b * C / 1e9
        print(f"| {name} | {gb(bf16):.2f} GB | {gb(fp8):.2f} GB | {gb(bf16-fp8):.2f} GB |")
...
def _bf16_absorbed_decode(q, pe, latent_bf16, scale):
    ...
    mask = pe[:, :, None, :].astype(latent_bf16.dtype)  # SDPA mask must match output dtype
...
def latency_table(B=1, H=128, D=512, group=DEFAULT_GROUP_SIZE):
    ...
    print("| context | bf16 absorbed SDPA (2× mem) | FP8 — mlx (default) | "
    ...
    for T in [1024, 4096, 16384, 65536]:
        ...
        t_mlx = _time(lambda: _mla_fp8_decode_reference(q, pe, codes, scales, scale, group))
...
def accuracy_table(B=1, H=32, D=512, group=DEFAULT_GROUP_SIZE):
    ...
    dists = {
        ...
        "N(0,1)+outliers": lambda s: mx.random.normal(s) * (1 + 6 * (mx.random.uniform(shape=s) > 0.97)),
        "heavy-tail (t)": lambda s: mx.random.normal(s) / mx.maximum(mx.abs(mx.random.normal(s)), 0.3),
    }
    for dname, gen in dists.items():
        for T in [2048, 16384]:
            ...
            pe = mx.random.normal((B, H, T)).astype(mx.float32)
            ker = _mla_fp8_decode_metal(q, pe, codes, scales, scale, group).astype(mx.float32)
            ref = _mla_fp8_decode_reference(q, pe, codes, scales, scale, group).astype(mx.float32)
            a, b = ker.reshape(-1), ref.reshape(-1)
            cos = float(mx.sum(a * b) / (mx.sqrt(mx.sum(a * a)) * mx.sqrt(mx.sum(b * b))))
    ...
    for dname, gen in dists.items():
        ...
        for g in (128, 64):
            ...
            rms = float(mx.sqrt(mx.mean((deq - latent) ** 2)) / mx.sqrt(mx.mean(latent**2)))
