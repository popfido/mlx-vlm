...
def test_cache_append_grow_and_offset():
    ...
    assert c.shape == (B, P, KV_LORA) and s.shape == (B, P, 4) and k.shape == (B, P, ROPE)
    for _ in range(5):
...
def test_cache_trim_then_grow_preserves_data():
    ...
    codes, scales, _ = cache.update_and_fetch(lat2, kpe2)  # forces grow with prev%step!=0
...
def test_attention_decode_matches_full_recompute(method):
    ...
    for i in range(steps):
        pos = P + i
        o = mla_fp8_attention(
            qa[:, :, pos : pos + 1], qpe[:, :, pos : pos + 1],
            lat[:, pos : pos + 1], kpe[:, pos : pos + 1], cache, SCALE, method=method,
        )
...
def test_attention_prefill_is_causal():
    ...
    assert _max_rel_err(full[:, :, -1:], full2[:, :, -1:]) > 1e-3  # last token did change
